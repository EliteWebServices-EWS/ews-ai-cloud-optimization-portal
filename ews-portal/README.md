...
```bash
ews-portal$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ews-portal$ sam local start-api
ews-portal$ curl http://localhost:3000/
```
...
```bash
ews-portal$ sam logs -n HelloWorldFunction --stack-name ews-portal --tail
```
...
```bash
sam delete --stack-name ews-portal
```
