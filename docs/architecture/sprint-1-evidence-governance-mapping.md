...
| **REPLACED** | Production implements the invariant through a different, documented mechanism. | Keep the production mechanism; define an adapter/translation only if Sprint 2 needs one. |
...
| --- | --- | --- | --- | --- | --- |
| I-01 | Evidence used for a decision must be retained with resource and collection context. | Durable discovery, EC2 Cost analysis runs/findings, and execution plans retain resource/run context. | No single cross-domain, versioned evidence envelope links every engine. | **IMPROVED** | Preserve domain persistence; define a non-destructive correlation contract in Sprint 2. |
...
| I-03 | Metrics must be sufficient for the observation being evaluated. | Legacy metric-series validation; Cost records expected/actual samples and a completeness state. | No shared sufficiency vocabulary across all plugins. | **REPLACED** | Keep production completeness assessment; Sprint 2 should normalize only its output, not replace collection logic. |
| I-04 | A recommendation must state the observation period behind its evidence. | Legacy window policy; Cost run/evidence records observation duration and timestamps. | Other recommendation domains do not yet expose the same durable window fields. | **REPLACED** | Preserve Cost run metadata and specify a reusable observation-window contract. |
...
| I-08 | A decision must distinguish an actionable recommendation from an unavailable target. | Legacy policy produces an advisory warning; Cost produces explainable findings and explicit insufficient-data outcomes. | Provider recommendation hints and Cost rule findings are not the same type. | **REPLACED** | Do not force one into the other; define an availability field that preserves source and disposition. |
...
| --- | --- | --- | --- | --- |
| Persistence | Tenant/account/resource identity; evidence/run/finding identifier; collection time; source version; lifecycle/run state. | Whether evidence is traceable, current, and safe to compare over time. | Discovery repositories; Cost run/finding records; execution plans/history. | Define correlation IDs and retention owner; do not move existing records. |
...
| --- | --- | --- | --- | --- |
| D-01 — metric sufficiency | Research invariant requires enough evidence; legacy governance evaluates generic metric arrays, while Cost evaluates provider-collected completeness for the requested window. This differs because production CloudWatch collection can fail or return partial data. | Yes — it preserves collection provenance and explicit incomplete states. | Normalize fact names only; retain domain evaluators. | No. |
| D-02 — observation evidence | The generic concept is a qualified observation period; Cost retains run-level duration and concrete collection boundaries, while legacy governance uses a policy check. This differs to support operational auditability. | Yes. | Define common window fields and provenance. | No. |
| D-03 — recommendation availability | A provider hint is optional in legacy governance; Cost creates advisory findings from controlled rules and represents insufficient data explicitly. This differs because a production finding is not equivalent to an external recommendation. | Yes — it avoids inventing an action when evidence is absent. | Define typed availability/disposition. | No. |
...
| D-07 — persistence shape | Commercial persistence is domain-specific and tenant-scoped rather than a single research-shaped object. This differs to preserve lifecycle, concurrency, audit, and tenant controls already in production paths. | Yes. | Design a link-only envelope; no record replacement/migration without approval. | No. |
...
4. The precedence order among policy failures, readiness, confidence, pricing status, recommendation availability, and durable approval.
