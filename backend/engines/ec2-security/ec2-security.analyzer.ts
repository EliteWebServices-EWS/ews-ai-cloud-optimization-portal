...
function analyzeOne(instance: Ec2SecurityInventoryItem, policy: Required<Ec2GovernancePolicy>): Ec2SecurityAnalysisResult {
  ...
  if (!name || !(new RegExp(policy.namingPattern).test(name))) governanceFindings.push({ check: 'naming_standard', severity: 'low', message: 'The Name tag does not meet the naming standard.', remediation: `Rename or retag the instance to match ${policy.namingPattern}.` });
...
export function analyzeEc2Security(inventory: Ec2SecurityInventoryItem[], overrides: Ec2GovernancePolicy = {}): Ec2SecurityAnalysisResponse {
  const policy = { ...DEFAULT_EC2_GOVERNANCE_POLICY, ...overrides };
