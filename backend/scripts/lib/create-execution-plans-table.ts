...
export interface CreateExecutionPlansTableOptions {
  tableName: string;
  endpoint?: string;
  region: string;
}

export function parseCreateExecutionPlansTableEnv(
  env: NodeJS.ProcessEnv = process.env,
): CreateExecutionPlansTableOptions {
  const tableName =
    env.EXECUTION_PLANS_TABLE_NAME?.trim() ||
    env.DYNAMODB_TABLE_NAME?.trim() ||
    'sisum-execution-plans-validation';
...
export function buildExecutionPlansCreateTableInput(tableName: string) {
  return {
    TableName: tableName,
...
export async function createExecutionPlansTable(
  client: DynamoDBClient,
  options: CreateExecutionPlansTableOptions,
): Promise<void> {
  try {
    await client.send(
      new CreateTableCommand(
        buildExecutionPlansCreateTableInput(options.tableName),
      ),
    );
  } catch (error) {
  ...
  if (described.Table?.TableStatus !== 'ACTIVE') {
    throw new Error(
      `Execution plans table ${options.tableName} is not ACTIVE.`,
    );
  }
}
